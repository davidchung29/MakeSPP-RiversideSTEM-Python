...
def redrawGameWindow():   #this function basicaly refreshes the screen to keep it updated
    win.blit(bg, (0, 0))     #draws the background at the center, or 0,0
...
#player = player(220,370,40,60)       #draws player at 150,370 with width 40 and height 60
wait = True
...
run = True                            # makes main game run True
...
while run:
    ...
    for event in pygame.event.get():       #checks for input
        if event.type == pygame.QUIT:        # if input is quit(the x at the top of the screen)
            run = False                         # if program recieves quit input, closes program
